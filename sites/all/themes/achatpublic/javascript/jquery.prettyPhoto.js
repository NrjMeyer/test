	...
	$.fn.prettyPhoto = function(pp_settings) {
		pp_settings = jQuery.extend({
			...
			markup: '<div class="pp_pic_holder"> \
						...
						<div class="pp_content_container" id="pp_content_container"> \
							<div class="pp_left"> \
							<div class="pp_right"> \
		...
		*/
		$.prettyPhoto.initialize = function() {
			...
			if(settings.init_url){
				$("#hidden_url_called").attr('value', $(this).attr('href'));
			}
			
			if(settings.init_url_documentation){
				$("#hidden_url_called_documentation").attr('value', $(this).attr('href'));
			}
			...
			$pp_pic_holder.fadeIn(function(){
				...
				switch(_getFileType(pp_images[set_position])){
					...
					case 'custom':
						...
						myClone = $(settings.custom_markup).clone().css({'width':settings.default_width}).wrapInner('<div id="pp_full_res"><div class="pp_inline clearfix"></div></div>').appendTo($('body'));
						correctSizes = _fitToViewport($(myClone).width(),$(myClone).height() - 20);
						$(myClone).remove();
						toInject = settings.inline_markup.replace(/{content}/g,$(settings.custom_markup).html());
		...
		*/
		...
		function _getFileType(itemSrc){
			if (itemSrc.match(/youtube\.com\/watch/i)) {
				...
				return 'iframe';
			}else if(itemSrc.indexOf('custom') != -1 || settings.custom_markup.substr(0,1) == '#'){
				return 'custom';
